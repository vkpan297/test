...
class Notification extends Model
{
    ...
    public function total_notifications()
    {
        ...
        else
        {
            // safe mode is on
            $select = array(
                            DB::raw("MIN($this->table.id) as id"),
                            DB::raw("COUNT($this->table.n_type) as total"),
                            "$this->table.n_type",
                            DB::raw("MIN($this->table.data) as data"),
                            DB::raw("MIN($this->table.read_at) as read_at"),
                            DB::raw("MIN($this->table.updated_at) as updated_at"),
                        );
        }
